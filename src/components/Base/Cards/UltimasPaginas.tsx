...
interface UltimasPaginasProps {
  menuPages: LinkProps[]; // array para renderizar
  setMenuPages: (pages: LinkProps[]) => void; // função para atualizar no pai
  refreshSignal: number;
}

const UltimasPaginas: React.FC<UltimasPaginasProps> = ({
  menuPages,
  setMenuPages,
  refreshSignal,
}) => {
  ...
  useEffect(() => {
    ...
    const fetchChamados = async () => {
      try {
        ...
        const sortedLinks = (parsedLinks.length ? parsedLinks : linksDb).sort(
          (a, b) => b.count - a.count
        );

        setMenuPages(sortedLinks);
      } catch (err) {
        setMenuPages(linksDb);
        console.error('Erro ao buscar páginas:', err);
      }
    };

    fetchChamados();
  }, [isLoading, token, setMenuPages, refreshSignal]);
      ...
      {menuPages.map((item, index) => (
        <div key={index} className="flex items-start gap-2 text-sm">
          ...
          <div className="flex flex-col">
            <span className={`${item.active ? 'font-semibold underline' : ''}`}>{item.title}</span>
