...
export const menuItens: MenuItem[] = [
  { text: "Dashboards", icon: <LayoutDashboard size={16} />, href: "#dashboard", countsPerUser: 0 },
  { text: "Usuários & Permissões", icon: <Users size={16} />, href: "#usuarios", countsPerUser: 0 },
  ...
  { text: "Alocação e posição", icon: <BarChartHorizontal size={16} />, href: "#alocacao", countsPerUser: 0 },
  ...
  { text: "Serviços", icon: <Settings size={16} />, href: "#servicos", countsPerUser: 0 },
  {
    ...
    href: "https://a5x-dev.4biz.one/4biz/pages/experienceCenterPreview/experienceCenterPreview.load?ecPreview=true#/ec?idExperienceCenter=1",
    countsPerUser: 0,
  },
  { text: "Formador de mercado", icon: <DollarSign size={16} />, href: "#formador", countsPerUser: 0 },
];
...
const Sidebar = ({ permissionRule, isCollapsed, pages, onCountUpdated }: SidebarProps) => {
  ...
  function getStableRandomMenu(menu: MenuItem[], permissionRule: string): MenuItem[] {
    ...
    const rng = seededRandom(parseInt(permissionRule, 36));
    const count = 1 + Math.floor(rng() * menu.length);
  ...
  return (
    <div
      className={`relative h-full bg-[#EDEDED] shadow z-10 transition-all duration-300 ${isCollapsed ? "w-16" : "w-64"
        }`}
      ref={sidebarRef}
    >
      <ul className="py-2">
            ...
            >
              ...
              {!isCollapsed && (
                <>
                  ...
                  {item.submenu && <ChevronRight size={16} className="ml-auto opacity-50" />}
                </>
              )}
            </div>
